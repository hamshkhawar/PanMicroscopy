...
output_file = Path(f'/projects/PanMicroscopy/NyxusFeatures/recursionpharma/combined_arrow/{dataset_name}/combine_arrowfile/comb_{dataset_name}_NyxusFeatures.arrow')
...
# Open the Vaex file only once if it exists
if output_file.exists():
    combined_df = vaex.open(output_file)
else:
    combined_df = None


def link_medatadata(df):
    ...
    df['experimental_technique'] = np.array(["high-dimensional human cellular assay for COVID-19 associated disease"] * len(df))
    ...
    # Filter columns
    combined_df = combined[metacols + varlist]
    del combined
    gc.collect()
...
df = vaex.open(f"/projects/PanMicroscopy/NyxusFeatures/recursionpharma/combined_arrow/{dataset_name}/combine_arrowfile/{dataset_name}_NyxusFeatures.arrow")
...
for chunk_idx in tqdm(range(num_chunks), desc="Processing chunks"):
    ...
    # Process and get the chunk as a Vaex DataFrame
    processed_chunk = link_medatadata(df_chunk)
    
    if combined_df is not None:
        # Append new chunk to existing Vaex DataFrame
        combined_df = vaex.concat([combined_df, processed_chunk])
    else:
    ...
    print(f"Successfully processed and exported chunk {chunk_idx + 1} of {num_chunks}.")
