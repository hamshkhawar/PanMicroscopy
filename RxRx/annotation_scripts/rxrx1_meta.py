...
out_dir = Path(f"/projects/PanMicroscopy/NyxusFeatures/recursionpharma/RxRx_combined/{dataset}")
...
for i, p in enumerate(arrowpaths):
    ...
    combined_pandas['dose'] = None
    combined_pandas['version'] = None
    combined_pandas['smiles'] = None
    combined_pandas['disease_condition'] = None
    combined_pandas['modality'] = 'Optical Imaging'
    combined_pandas['experimental_technique'] = 'Training Models to Identify siRNA from Unseen Experimental Data'
    ...
    combined_pandas.rename(columns={'experiment_x': 'experiment', 
                                                      ...
                                                      'cell_type':'cell',
                                                      'sirna':'perturbation',
                                                      'sirna_id': 'perturbation_id',
                                                      'well_type':'control_type'
    ...
    with pa.OSFile(str(output_file), 'wb') as f:
        ...
        with pa.ipc.new_file(f, combined_table.schema, options=options) as writer:
